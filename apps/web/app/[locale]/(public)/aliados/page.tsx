...
export default async function PartnersLandingPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return <div className="overflow-hidden bg-white pb-20 text-[#111418]">
    <section className="relative border-b border-black/[.05] bg-[#fbfcfb] px-6 py-16 sm:px-8 lg:py-24">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_78%_20%,rgba(0,194,199,.15),transparent_32%),linear-gradient(to_bottom,#fbfcfb_0%,#f7fbfa_70%,#ffffff_100%)]" aria-hidden="true"/>
      <div className="premium-grid absolute inset-0 opacity-20" aria-hidden="true"/>
      <div className="relative mx-auto grid max-w-7xl gap-12 lg:grid-cols-[1.08fr_.92fr] lg:items-center">
        <div>
          <p className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#00aeb3]">{es ? 'DESPLIEGUE EMPRESARIAL DE IA · ALIANZAS ESTRATÉGICAS' : 'ENTERPRISE AI DEPLOYMENT · STRATEGIC PARTNERSHIPS'}</p>
          <h1 className="mt-6 max-w-5xl font-[var(--font-display)] text-5xl font-semibold leading-[.98] tracking-[-0.06em] text-[#111418] sm:text-6xl lg:text-7xl">{es ? 'La IA no transforma una empresa hasta que transforma su operación.' : 'AI does not transform an enterprise until it transforms its operations.'}</h1>
          <p className="mt-7 max-w-3xl text-lg leading-8 text-[#68717b] sm:text-xl">{es ? 'Trends172Tech trabaja junto a líderes y equipos operativos para convertir procesos complejos en sistemas inteligentes, automatizados, medibles y listos para producción.' : 'Trends172Tech works alongside leaders and operating teams to turn complex workflows into intelligent, automated, measurable, production-ready systems.'}</p>
          <div className="mt-9 flex flex-wrap gap-3"><Link href={enterpriseMail} className="rounded-full bg-[#111418] px-7 py-3.5 text-sm font-semibold text-white shadow-[0_14px_35px_rgba(17,20,24,.12)] transition hover:-translate-y-0.5 hover:bg-[#00aeb3]">{es ? 'Solicitar diagnóstico ejecutivo' : 'Request an executive assessment'}<span className="ml-3 text-[#4de0dc]">→</span></Link><Link href="#alianza" className="rounded-full border border-black/10 bg-white/88 px-7 py-3.5 text-sm font-semibold text-[#20252b] shadow-[0_10px_28px_rgba(17,20,24,.05)] transition hover:border-[#00aeb3]/45 hover:text-[#008f94]">{es ? 'Quiero ser aliado' : 'Become a partner'}<span className="ml-3 text-[#00aeb3]">→</span></Link></div>
        </div>
        <div className="rounded-[32px] border border-black/[.06] bg-white/90 p-5 shadow-[0_28px_90px_rgba(20,70,70,.09)] backdrop-blur sm:p-7">
          <div className="flex items-center justify-between border-b border-black/[.06] pb-5"><div><p className="text-xs uppercase tracking-[.2em] text-[#8a9199]">Trends172Tech</p><p className="mt-1 font-semibold">Enterprise AI Deployment System</p></div><span className="rounded-full border border-[#00c2c7]/20 bg-[#eafffb] px-3 py-1 text-xs font-semibold text-[#00aeb3]">FDE</span></div>
          <div className="mt-5 grid gap-3 sm:grid-cols-2">{(es ? ['Estrategia y resultados','Procesos críticos','Datos e integraciones','Personas y adopción','Seguridad y control','Medición y escala'] : ['Strategy and outcomes','Critical workflows','Data and integrations','People and adoption','Security and control','Measurement and scale']).map((item,index)=><div key={item} className="rounded-2xl border border-black/[.06] bg-[linear-gradient(145deg,#ffffff,#f6fbfa)] p-4"><span className="text-xs font-semibold text-[#00aeb3]">0{index+1}</span><p className="mt-3 text-sm font-medium text-[#3f4650]">{item}</p></div>)}</div>
          <p className="mt-5 border-t border-black/[.06] pt-5 text-sm leading-6 text-[#68717b]">{es ? 'Un expediente vivo por iniciativa: evidencia, decisiones, riesgos, documentos, responsables y próximas acciones.' : 'A living record for every initiative: evidence, decisions, risks, documents, owners, and next actions.'}</p>
        </div>
      </div>
    </section>
    ...
    <section className="border-y border-black/[.05] bg-[#f5fbfa] px-6 py-24 sm:py-32"><div className="mx-auto max-w-7xl"><p className="text-[11px] font-semibold uppercase tracking-[.22em] text-[#00aeb3]">{es ? 'CAPACIDAD EJECUTIVA + ENTREGA TÉCNICA' : 'EXECUTIVE CAPABILITY + TECHNICAL DELIVERY'}</p><h2 className="mt-4 max-w-4xl font-[var(--font-display)] text-4xl font-semibold tracking-[-.05em] sm:text-5xl">{es ? 'Todo lo necesario para pasar de intención a ventaja operativa.' : 'Everything required to move from intent to operational advantage.'}</h2><div className="mt-10 grid gap-4 md:grid-cols-2 lg:grid-cols-3">{capabilities.map(([title,body],index)=><article key={title} className="rounded-[28px] border border-black/[.06] bg-white p-8 shadow-[0_20px_65px_rgba(17,20,24,.045)]"><div className="flex items-center justify-between"><span className="text-xs font-semibold text-[#00aeb3]">0{index+1}</span><span className="h-9 w-9 rounded-full border border-[#00c2c7]/20 bg-[#eafffb]"/></div><h3 className="mt-10 text-xl font-semibold tracking-[-.02em]">{title}</h3><p className="mt-3 text-sm leading-7 text-[#68717b]">{body}</p></article>)}</div></div></section>

    <section className="mx-auto max-w-7xl px-6 py-24 sm:py-32"><div className="grid gap-8 rounded-[36px] border border-black/[.05] bg-[linear-gradient(145deg,#ffffff,#f4fbfa)] p-8 shadow-[0_24px_80px_rgba(20,60,60,.06)] lg:grid-cols-2 lg:p-12"><div><p className="text-[11px] font-semibold uppercase tracking-[.22em] text-[#00aeb3]">{es ? 'SISTEMA DE TRABAJO COMPARTIDO' : 'SHARED DELIVERY SYSTEM'}</p><h2 className="mt-4 font-[var(--font-display)] text-4xl font-semibold tracking-[-.05em] text-[#111418]">{es ? 'Cada oportunidad conserva su propia memoria empresarial.' : 'Every opportunity keeps its own business memory.'}</h2><p className="mt-5 text-base leading-8 text-[#68717b]">{es ? 'Nuestro Portal de Aliados organiza investigación, levantamientos EFICI, documentos, propuestas, reuniones, pendientes y decisiones dentro de un expediente independiente por proyecto.' : 'Our Partner Portal organises research, EFICI discovery, documents, proposals, meetings, pending actions, and decisions inside an independent record for every project.'}</p></div><div className="grid gap-3 sm:grid-cols-2">{(es ? [['Hechos','Información confirmada y con fuente.'],['Supuestos','Hipótesis visibles, nunca presentadas como verdad.'],['Riesgos','Bloqueos operativos, técnicos o de adopción.'],['Próximas acciones','Responsabilidad y continuidad del proyecto.']] : [['Facts','Confirmed, sourced information.'],['Assumptions','Visible hypotheses, never presented as truth.'],['Risks','Operational, technical, or adoption blockers.'],['Next actions','Ownership and project continuity.']]).map(([title,body])=><div key={title} className="rounded-2xl border border-black/[.06] bg-white/90 p-5"><h3 className="font-semibold">{title}</h3><p className="mt-2 text-sm leading-6 text-[#68717b]">{body}</p></div>)}</div></div></section>

    <section id="alianza" className="mx-auto max-w-7xl scroll-mt-24 px-6 pb-24"><div className="grid gap-5 lg:grid-cols-2"><article className="rounded-[30px] border border-black/[.06] bg-white p-8 shadow-[0_20px_65px_rgba(17,20,24,.045)] lg:p-12"><p className="text-[11px] font-semibold uppercase tracking-[.22em] text-[#00aeb3]">{es ? 'PARA EMPRESAS' : 'FOR ENTERPRISES'}</p><h2 className="mt-4 font-[var(--font-display)] text-3xl font-semibold tracking-[-.04em]">{es ? 'Buscamos un proceso crítico que merezca ser transformado.' : 'We look for a critical workflow worth transforming.'}</h2><p className="mt-4 leading-7 text-[#68717b]">{es ? 'Ideal para organizaciones con patrocinio ejecutivo, acceso a los responsables del proceso y disposición para medir resultados reales.' : 'Designed for organisations with executive sponsorship, access to process owners, and a willingness to measure real outcomes.'}</p><Link href={enterpriseMail} className="mt-7 inline-flex rounded-full bg-[#111418] px-6 py-3 text-sm font-semibold text-white transition hover:bg-[#00aeb3]">{es ? 'Presentar un desafío empresarial' : 'Present a business challenge'}</Link></article><article className="rounded-[30px] border border-[#00c2c7]/15 bg-[#f4fbfa] p-8 text-[#111418] shadow-[0_20px_65px_rgba(20,70,70,.055)] lg:p-12"><p className="text-[11px] font-semibold uppercase tracking-[.22em] text-[#00aeb3]">{es ? 'PARA ALIADOS' : 'FOR PARTNERS'}</p><h2 className="mt-4 font-[var(--font-display)] text-3xl font-semibold tracking-[-.04em]">{es ? 'Construyamos una capacidad conjunta de transformación.' : 'Let’s build a joint transformation capability.'}</h2><p className="mt-4 leading-7 text-[#68717b]">{es ? 'Buscamos consultoras, especialistas sectoriales, integradores y líderes comerciales que aporten confianza, contexto y acceso a oportunidades donde podamos ejecutar juntos.' : 'We seek consultancies, industry specialists, integrators, and commercial leaders who bring trust, context, and access to opportunities where we can deliver together.'}</p><Link href={partnerMail} className="mt-7 inline-flex rounded-full bg-[#111418] px-6 py-3 text-sm font-semibold text-white transition hover:bg-[#00aeb3]">{es ? 'Quiero ser aliado' : 'Become a partner'}</Link></article></div></section>
