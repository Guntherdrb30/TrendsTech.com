  ...
  return (
    <div className="grid gap-6 xl:grid-cols-[0.72fr_1.28fr]">
      ...
      <section className="rounded-[26px] border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-950">
        ...
        {projects.length === 0 ? <div className="mt-8 grid min-h-[360px] place-items-center rounded-2xl border border-dashed border-slate-300 bg-slate-50/60 p-8 text-center dark:border-slate-700 dark:bg-slate-900/30"><div className="max-w-md"><div className="mx-auto grid h-14 w-14 place-items-center rounded-2xl bg-slate-950 text-xl font-semibold text-white dark:bg-white dark:text-slate-950">+</div><h4 className="mt-5 text-lg font-semibold">Todavía no hay proyectos reales</h4><p className="mt-2 text-sm leading-6 text-slate-500">El primer proyecto creado desde el formulario aparecerá aquí y quedará almacenado en Neon.</p><Link href={`/${locale}/admin/programming/projects/new`} className="mt-5 inline-flex text-sm font-semibold text-cyan-700 dark:text-cyan-300">Crear primer proyecto →</Link></div></div> : <div className="mt-6 space-y-3">{projects.map((project) => <article key={project.id} className="grid gap-4 rounded-2xl border border-slate-200 p-5 transition hover:border-cyan-300 hover:bg-cyan-50/30 dark:border-slate-800 dark:hover:border-cyan-900 dark:hover:bg-cyan-950/10 lg:grid-cols-[1fr_130px_150px_130px]">
          <div className="min-w-0"><Link href={`/${locale}/admin/programming/projects/${project.id}`} className="font-semibold text-slate-950 hover:text-cyan-700 dark:text-white dark:hover:text-cyan-300">{project.name}</Link><p className="mt-1 text-xs text-slate-500">{project.clientName || 'Proyecto interno'} · {project.mode} · {project.stage}</p>{project.summary && <p className="mt-2 line-clamp-2 text-xs leading-5 text-slate-500">{project.summary}</p>}{project.vercelProjectName && <div className="mt-3 flex flex-wrap items-center gap-2"><span className="truncate rounded-full bg-slate-100 px-3 py-1.5 font-mono text-[11px] text-slate-700 dark:bg-slate-900 dark:text-slate-200">Vercel: {project.vercelProjectName}</span><a href={vercelProjectDashboardUrl(project.vercelProjectName)} target="_blank" rel="noreferrer" className="rounded-full bg-slate-950 px-3 py-1.5 text-[11px] font-semibold text-white hover:bg-cyan-700 dark:bg-white dark:text-slate-950">Abrir en Vercel ↗</a></div>}</div>
          <div><p className="text-[11px] uppercase tracking-wide text-slate-400">Forecast</p><p className="mt-1 text-sm font-semibold">{money(project.forecastCost)}</p></div>
          <div><p className="text-[11px] uppercase tracking-wide text-slate-400">Valor referencia</p><p className="mt-1 text-sm font-semibold">{money(project.contractedValue)}</p></div>
          <div><p className="text-[11px] uppercase tracking-wide text-slate-400">Blueprint</p><p className="mt-1 text-xs font-semibold text-cyan-700 dark:text-cyan-300">{project.blueprintStatus || '—'}</p><Link href={`/${locale}/admin/programming/projects/${project.id}`} className="mt-3 inline-flex text-xs font-semibold text-slate-600 hover:text-cyan-700 dark:text-slate-300">Ver proyecto →</Link></div>
        </article>)}</div>}
      </section>
    </div>
  );
