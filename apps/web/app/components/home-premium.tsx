...
function Hero({ locale, t }: { locale: string; t: LocaleCopy }) {
  const reduce = useReducedMotion();
  return <section className="relative isolate overflow-hidden bg-[#fbfcfb] pt-16 sm:pt-20 lg:min-h-[88vh] lg:pt-24">
    <motion.div aria-hidden="true" className="absolute -right-28 top-8 -z-10 h-[430px] w-[430px] rounded-full bg-[radial-gradient(circle,rgba(0,194,199,.18),rgba(167,243,233,.09)_42%,transparent_72%)] blur-3xl sm:h-[600px] sm:w-[600px] lg:right-[2%] lg:top-[4%]" animate={reduce ? undefined : { scale: [0.94, 1.06, 0.94], opacity: [0.45, 0.75, 0.45] }} transition={{ duration: 11, repeat: Infinity, ease: 'easeInOut' }} />
    <div className="absolute inset-0 -z-20 bg-[linear-gradient(to_bottom,#fbfcfb_0%,#f7fbfa_62%,#ffffff_100%)]" />
    <div className="relative mx-auto max-w-7xl px-6 pb-8 sm:pb-10 lg:grid lg:min-h-[calc(88vh-96px)] lg:grid-cols-[.94fr_1.06fr] lg:items-center lg:gap-12 lg:pb-14">
      <div className="relative z-10 max-w-2xl py-5 sm:py-8 lg:py-0">
        <motion.div initial={reduce ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="mb-5 flex items-center gap-3 sm:mb-7">
          ...
          <p className="text-sm font-medium text-[#66717a]">{t.creator}</p>
        </motion.div>
        ...
        <motion.p initial={reduce ? false : { opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .18, duration: .7 }} className="mt-5 max-w-[19ch] text-[27px] font-medium leading-[1.04] tracking-[-.045em] text-[#20252b] sm:mt-6 sm:max-w-2xl sm:text-4xl">{t.platform}</motion.p>
        ...
        <motion.div initial={reduce ? false : { opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .38 }} className="mt-7 flex max-w-md flex-col gap-3 sm:mt-9 sm:flex-row sm:flex-wrap">
          <Link href={`/${locale}/systems/luna`} className="rounded-full bg-[#111418] px-7 py-3.5 text-center text-sm font-semibold text-white shadow-[0_14px_35px_rgba(17,20,24,.12)] transition hover:-translate-y-0.5 hover:bg-[#00aeb3]">{t.discover}<span className="ml-3 text-[#4de0dc]">→</span></Link>
          <Link href={`/${locale}/contact`} className="rounded-full border border-black/10 bg-white/88 px-7 py-3.5 text-center text-sm font-semibold text-[#20252b] shadow-[0_10px_28px_rgba(17,20,24,.05)] backdrop-blur transition hover:border-[#00aeb3]/45 hover:text-[#008f94]">{t.demo}<span className="ml-3 text-[#00aeb3]">→</span></Link>
        </motion.div>
      </div>
      <motion.div className="pointer-events-none absolute right-[-130px] top-[250px] h-[310px] w-[310px] opacity-[.34] min-[390px]:right-[-112px] min-[390px]:top-[265px] min-[390px]:h-[345px] min-[390px]:w-[345px] sm:right-[-40px] sm:top-[170px] sm:h-[460px] sm:w-[460px] sm:opacity-[.46] lg:pointer-events-auto lg:relative lg:right-auto lg:top-auto lg:h-auto lg:w-auto lg:opacity-100" initial={reduce ? false : { opacity: 0, scale: .97, y: 18 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ delay: .2, duration: 1 }}>
        <LunaSystemMap nodes={t.architectureNodes} decorative className="h-full w-full" />
      </motion.div>
    </div>
    <div className="relative z-10 border-y border-black/[.05] bg-white/88 backdrop-blur-xl"><div className="mx-auto grid max-w-7xl grid-cols-2 gap-px px-6 py-4 sm:flex sm:items-center sm:gap-7 lg:px-6"><span className="col-span-2 mb-1 text-[10px] font-semibold uppercase tracking-[.18em] text-[#8a9199] sm:mb-0 sm:mr-auto sm:max-w-[180px]">{t.proof}</span>{t.proofItems.map((item, index) => <span key={item} className="flex items-center gap-2 border-l border-black/[.06] py-2 pl-3 text-[11px] text-[#515862] sm:border-0 sm:py-0 sm:pl-0 sm:text-sm"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#e9fbf8] text-[10px] font-semibold text-[#00aeb3]">0{index + 1}</span>{item}</span>)}</div></div>
  </section>;
}
