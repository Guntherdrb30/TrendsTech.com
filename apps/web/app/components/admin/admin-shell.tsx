      ...
      <div className="grid min-h-screen lg:grid-cols-[268px_1fr]">
        <aside className="border-b border-black/8 bg-white/95 px-4 py-4 dark:border-slate-800 dark:bg-slate-950 lg:sticky lg:top-0 lg:h-screen lg:border-b-0 lg:border-r lg:px-5 lg:py-6">
          <div className="flex items-center justify-between gap-3 lg:block">
            <Link href={`${base}/admin`} className="block">
              <div className="flex items-center gap-3">
                ...
                <span><span className="block text-sm font-semibold text-slate-950 dark:text-white">Trends172 Tech</span><span className="mt-0.5 block text-xs text-slate-500 dark:text-slate-400">{labels.title}</span></span>
              </div>
            </Link>
            <Link href={base} className="rounded-full border border-black/10 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 dark:border-slate-800 dark:text-slate-200 dark:hover:bg-slate-900 lg:mt-6 lg:inline-flex">{labels.backToSite}</Link>
          </div>
          <div className="mt-5">
            <p className="mb-3 text-[11px] font-semibold uppercase text-slate-500 dark:text-slate-400">{labels.navigation}</p>
            <nav className="flex gap-2 overflow-x-auto lg:flex-col lg:overflow-visible">
              {operationsNavItems.map((item) => {
                const href = `${base}${item.href}`;
                const active = pathname === href || (item.href !== '/admin' && pathname.startsWith(href));
                const label = item.key === 'partners' ? (locale.startsWith('es') ? 'Aliados' : 'Partners') : item.key === 'bookings' ? (locale.startsWith('es') ? 'Reservas' : 'Bookings') : item.key === 'engineeringStudio' ? 'Engineering Studio' : item.key === 'lunaImplementations' ? labels.lunaImplementations ?? 'Implementaciones LUNA' : labels[item.key];
          ...
          <div className="mt-6 border-t border-black/8 pt-5 dark:border-slate-800">
            <p className="mb-3 text-[11px] font-semibold uppercase text-slate-500 dark:text-slate-400">{labels.systemNavigation}</p>
            <nav className="flex gap-2 overflow-x-auto lg:flex-col lg:overflow-visible">
              {systemNavItems.map((item) => {
                const href = `${base}${item.href}`;
                const path = href.split('#')[0];
                const active = pathname === path || (item.key === 'siteMedia' && pathname.startsWith(path));
                return <Link key={item.key} href={href} className={cn('whitespace-nowrap rounded-lg border px-3 py-2 text-sm font-medium transition lg:w-full', active ? 'border-cyan-200 bg-cyan-50 text-cyan-800 dark:border-cyan-900 dark:bg-cyan-950/40 dark:text-cyan-200' : 'border-transparent text-slate-600 hover:border-black/10 hover:bg-slate-50 hover:text-slate-950 dark:text-slate-300 dark:hover:border-slate-800 dark:hover:bg-slate-900 dark:hover:text-white')}>{labels[item.key]}</Link>;
              })}
            </nav>
          </div>
        </aside>
        <div className="min-w-0">
          <header className="border-b border-black/8 bg-white/85 px-4 py-5 backdrop-blur dark:border-slate-800 dark:bg-slate-950/80 sm:px-6 lg:px-8">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-2xl font-semibold text-slate-950 dark:text-white">{labels.title}</h1><p className="mt-1 max-w-3xl text-sm text-slate-500 dark:text-slate-400">{labels.subtitle}</p></div><div className="rounded-full border border-cyan-200 bg-cyan-50 px-3 py-1.5 text-xs font-semibold text-cyan-800 dark:border-cyan-900 dark:bg-cyan-950/40 dark:text-cyan-200">{locale.startsWith('es') ? 'Administrador ROOT' : 'ROOT administrator'}</div></div>
          </header>
