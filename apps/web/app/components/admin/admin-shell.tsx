          ...
          <div className="mt-5">
            ...
            <nav className="flex gap-2 overflow-x-auto lg:flex-col lg:overflow-visible">
              {operationsNavItems.map((item) => {
                ...
                const label = item.key === 'engineeringStudio'
                  ? (locale.startsWith('es') ? 'Engineering Studio' : 'Engineering Studio')
                  : item.key === 'lunaImplementations'
                    ? labels.lunaImplementations ?? 'Implementaciones LUNA'
                    : labels[item.key];
        ...
        <div className="min-w-0">
          <header className="border-b border-black/8 bg-white/85 px-4 py-5 backdrop-blur dark:border-slate-800 dark:bg-slate-950/80 sm:px-6 lg:px-8">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div><h1 className="text-2xl font-semibold text-slate-950 dark:text-white">{labels.title}</h1><p className="mt-1 max-w-3xl text-sm text-slate-500 dark:text-slate-400">{labels.subtitle}</p></div>
              <div className="rounded-full border border-cyan-200 bg-cyan-50 px-3 py-1.5 text-xs font-semibold text-cyan-800 dark:border-cyan-900 dark:bg-cyan-950/40 dark:text-cyan-200">{locale.startsWith('es') ? 'Administrador ROOT' : 'ROOT administrator'}</div>
            </div>
          </header>
