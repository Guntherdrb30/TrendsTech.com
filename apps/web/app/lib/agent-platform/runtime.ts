...
export type PilotAgentRequest = {
  tenantId: string; agentInstanceId: string; sessionId: string; message: string; channel?: AgentChannel;
  userId?: string; projectId?: string; endCustomerId?: string;
};
...
export function contextFromRequest(request: PilotAgentRequest): AgentExecutionContext {
  const context: AgentExecutionContext = {
    tenantId: request.tenantId, deploymentId: CARPIHOGAR_PILOT_DEPLOYMENT_ID, agentInstanceId: request.agentInstanceId,
    sessionId: request.sessionId, channel: request.channel ?? 'api', userId: request.userId,
    projectId: request.projectId, endCustomerId: request.endCustomerId
  };
...
export async function inspectCarpiHogarPilot() {
  ...
  return { deploymentId: CARPIHOGAR_PILOT_DEPLOYMENT_ID, mcp: CARPIHOGAR_MCP.key, discoveredToolCount: tools.length,
    tools: tools.map((tool) => ({ name: tool.name, description: tool.description, annotations: tool.annotations })) };
}
export async function runCarpiHogarPilotTool(args: { request: PilotAgentRequest; toolName: string; input: Record<string, unknown> }) {
  ...
  try {
    ...
    return { runId: runTrace.runId, context, mcp: CARPIHOGAR_MCP.key, toolName: args.toolName, durationMs, result, trace };
  } catch (error) {
    runTrace.addTool({ toolName: args.toolName, durationMs: Date.now() - startedAt, status: 'FAILED', error: error instanceof Error ? error.message : String(error) });
