...
function createMcpAgentTool(context: AgentExecutionContext, runTrace: RunTrace, toolName: string, description: string) {
  return tool({ name: toolName, description, parameters: jsonInputSchema, execute: async ({ input }) => {
...
function buildTools(context: AgentExecutionContext, runTrace: RunTrace) {
  return CARPIHOGAR_MCP.tools.filter((descriptor) => descriptor.approval !== 'human_required')
...
export async function runCarpiHogarAgent(args: { request: PilotAgentRequest; context: AgentExecutionContext }) {
  ...
  const agent = new Agent({ name: 'CarpiHogar AI - Trends172 Pilot', model: MODEL, instructions: [
  ...
  try {
    ...
    runTrace.setUsage({ requests: result.state.usage.requests, inputTokens: result.state.usage.inputTokens,
      outputTokens: result.state.usage.outputTokens, totalTokens: result.state.usage.totalTokens,
    ...
    return { runId: runTrace.runId, context: args.context, model: MODEL, durationMs: Date.now() - startedAt,
      output: result.finalOutput ?? '', lastResponseId: result.lastResponseId ?? null, trace };
  } catch (error) {
