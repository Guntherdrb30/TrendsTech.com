...
export type AgentToolTrace = { toolName: string; durationMs: number; status: 'SUCCESS' | 'FAILED'; error?: string };
export type AgentRunStatus = 'SUCCESS' | 'FAILED';
export type AgentRunSummary = {
  id: string; createdAt: Date; tenantId: string | null; actorUserId: string; status: AgentRunStatus;
  mode: 'agent' | 'deterministic'; deploymentId: string; agentInstanceId: string; sessionId: string; channel: string;
  model: string | null; durationMs: number; toolCount: number; tools: AgentToolTrace[]; tokensIn: number | null;
  tokensOut: number | null; totalTokens: number | null; costUsdMicros: number | null; responseId: string | null; error: string | null;
};
...
export function createAgentRunTrace(args: { context: AgentExecutionContext; mode: 'agent' | 'deterministic'; model?: string | null }) {
  ...
  async function persistNativeUsage() {
    ...
    await prisma.tokenUsageLog.create({ data: {
      tenantId: args.context.tenantId, agentInstanceId: args.context.agentInstanceId, sessionId: args.context.sessionId,
      tokensIn: nativeUsage.inputTokens, tokensOut: nativeUsage.outputTokens, totalTokens: nativeUsage.totalTokens,
      costUsdMicros: cost.billableCostUsdMicros, model: args.model ?? null
    }});
  }
  async function usage() {
    const rows = await prisma.tokenUsageLog.findMany({ where: {
      tenantId: args.context.tenantId, agentInstanceId: args.context.agentInstanceId, sessionId: args.context.sessionId,
    ...
    return rows.reduce((acc, row) => ({
      tokensIn: (acc.tokensIn ?? 0) + (row.tokensIn ?? 0), tokensOut: (acc.tokensOut ?? 0) + (row.tokensOut ?? 0),
      totalTokens: (acc.totalTokens ?? 0) + row.totalTokens, costUsdMicros: (acc.costUsdMicros ?? 0) + (row.costUsdMicros ?? 0)
  ...
  async function finish(status: AgentRunStatus, extra?: { responseId?: string | null; error?: unknown }) {
    ...
    const meta: RunMeta = {
      runId, status, mode: args.mode, deploymentId: args.context.deploymentId, agentInstanceId: args.context.agentInstanceId,
      sessionId: args.context.sessionId, channel: args.context.channel, model: args.model ?? null,
      durationMs: Date.now() - startedAt.getTime(), toolCount: tools.length, tools, ...measuredUsage,
      responseId: extra?.responseId ?? null, error: extra?.error ? safeError(extra.error) : null
    };
    await prisma.auditLog.create({ data: {
      actorUserId: args.context.userId!, tenantId: args.context.tenantId,
      action: status === 'SUCCESS' ? 'agent.run.completed' : 'agent.run.failed', entity: 'agent_run', entityId: runId, metaJson: meta
    }});
    return meta;
  }
  return {
