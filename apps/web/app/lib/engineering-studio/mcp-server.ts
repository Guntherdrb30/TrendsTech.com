...
export function createTrendsMcpServer(actorRef = 'chatgpt-mcp') {
  ...
  server.registerTool('search', { title:'Buscar proyectos de Engineering Studio', description:'Find a Trends Engineering Studio project by name, client, stage, or status.', inputSchema:{query:z.string().min(1).max(200)}, annotations:{readOnlyHint:true,destructiveHint:false,openWorldHint:false} }, async ({query})=>{
  ...
  server.registerTool('fetch', { title:'Leer proyecto de Engineering Studio', description:'Load current Blueprint, stage, repository, approval state and recent Project Vault entries.', inputSchema:{id:z.string().uuid()}, annotations:{readOnlyHint:true,destructiveHint:false,openWorldHint:false} }, async ({id})=>{
