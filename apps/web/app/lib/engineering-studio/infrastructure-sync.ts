...
type VercelGitRepo = { type?: string; repo?: string; org?: string; repoId?: number | string; productionBranch?: string; };
type VercelProject = { id: string; name: string; framework?: string | null; updatedAt?: number; link?: VercelGitRepo | null; };
type VercelDeployment = { uid?: string; id?: string; url?: string | null; state?: string; readyState?: string; target?: string | null; created?: number; createdAt?: number; meta?: Record<string, string | undefined>; };
type ProjectIntegration = { id: string; projectId: string; externalProjectId: string; repositoryFullName: string | null; productionDeploymentId: string | null; productionCommitSha: string | null; productionState: string | null; };

function config() { const token = process.env.VERCEL_STUDIO_TOKEN || process.env.VERCEL_TOKEN; const teamId = process.env.VERCEL_STUDIO_TEAM_ID || process.env.VERCEL_TEAM_ID; if (!token) throw new Error('Falta VERCEL_STUDIO_TOKEN (o VERCEL_TOKEN).'); return { token, teamId }; }
async function vercelGet<T>(path: string, query: Record<string,string|number|undefined> = {}) { const { token, teamId } = config(); const url = new URL(`https://api.vercel.com${path}`); if (teamId) url.searchParams.set('teamId', teamId); for (const [key,value] of Object.entries(query)) if (value !== undefined) url.searchParams.set(key,String(value)); const response = await fetch(url, { headers: { Authorization: `Bearer ${token}` }, cache: 'no-store', signal: AbortSignal.timeout(20000) }); if (!response.ok) throw new Error(`Vercel API ${response.status}: ${await response.text()}`); return response.json() as Promise<T>; }
async function listAllVercelProjects() { const projects: VercelProject[] = []; let from: string | undefined; for (let page=0; page<50; page++) { const data = await vercelGet<{projects?:VercelProject[];pagination?:{next?:string|number|null}}>('/v10/projects',{limit:100,from}); projects.push(...(data.projects || [])); const next = data.pagination?.next; if (!next) break; from = String(next); } return projects; }
async function latestProductionDeployment(projectId: string) { const data = await vercelGet<{deployments?:VercelDeployment[]}>('/v7/deployments',{projectId,target:'production',limit:1}); return data.deployments?.[0] || null; }
function repositoryInfo(project: VercelProject, deployment: VercelDeployment | null) { const meta = deployment?.meta || {}; const org = project.link?.org || meta.githubCommitOrg; const repo = project.link?.repo || meta.githubCommitRepo; const fullName = org && repo ? `${org}/${repo}` : null; return { fullName, url: fullName ? `https://github.com/${fullName}` : null, branch: project.link?.productionBranch || meta.githubCommitRef || null, sha: meta.githubCommitSha || null }; }
async function existingIntegration(externalProjectId: string) { const rows = await prisma.$queryRaw<ProjectIntegration[]>(Prisma.sql`SELECT "id","projectId","externalProjectId","repositoryFullName","productionDeploymentId","productionCommitSha","productionState" FROM "StudioProjectIntegration" WHERE "provider"='VERCEL' AND "externalProjectId"=${externalProjectId} LIMIT 1`); return rows[0] || null; }
async function findStudioProject(repositoryUrl: string | null, name: string) { const rows = await prisma.$queryRaw<Array<{id:string}>>(Prisma.sql`SELECT "id" FROM "StudioProject" WHERE (${repositoryUrl}::text IS NOT NULL AND lower(COALESCE("repositoryUrl",''))=lower(${repositoryUrl}::text)) OR lower("name")=lower(${name}) ORDER BY "createdAt" ASC LIMIT 1`); return rows[0]?.id || null; }
async function createRecoveredProject(project: VercelProject, repositoryUrl: string | null, deployment: VercelDeployment | null) { const state = deployment?.state || deployment?.readyState || 'UNKNOWN'; const summary = `Proyecto descubierto automáticamente en Vercel. Estado de producción al importar: ${state}. ${repositoryUrl ? `Repositorio detectado: ${repositoryUrl}.` : 'Repositorio no identificado todavía.'}`; const created = await createStudioProject({ origin:'recovery', name:project.name, summary, marginPercent:0, commercialBudget:0, localAiRequired:false, repositoryUrl:repositoryUrl || undefined }); await addVaultEntry({ projectId:created.projectId, type:'NOTE', title:'Proyecto descubierto por Infrastructure Sync', content:summary, source:'ENGINEERING_STUDIO', sourceRef:`vercel:${project.id}`, meta:{provider:'VERCEL',externalProjectId:project.id} }); return created.projectId; }
  ...
  try {
    ...
    for (const external of projects) {
      try {
        ...
        const integration = await existingIntegration(external.id);
        ...
        if (changed && integration) await addVaultEntry({ projectId, type:'DEPLOYMENT', title:'Vercel producción actualizado', content:`Deployment ${deploymentId || 'sin id'} · ${productionState || 'UNKNOWN'} · commit ${repo.sha || 'no identificado'}`, source:'ENGINEERING_STUDIO', sourceRef:`vercel-deployment:${deploymentId || external.id}`, meta:{provider:'VERCEL',deploymentId,deploymentUrl,productionState,commitSha:repo.sha,branch:repo.branch} });
        updatedCount++;
      } catch (error) { errorCount++; errors.push({project:external.name,error:error instanceof Error ? error.message : String(error)}); }
    }
